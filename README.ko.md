...
- **core 규칙**(증거·범위·판정·멈춤·시크릿 마스킹·디버깅·격리)은 `SessionStart` 훅이 세션마다 주입합니다.
...
| 층 | 내용 | 언제 로드되나 | 스코프 |
|---|---|---|---|
| **core** | 증거 기반 완료, 범위 고정, 완료 판정 주체, 중단 조건(2회 실패·블로커·파괴적 작업), 시크릿 마스킹, 디버깅 진입점, 격리 | 세션 시작 시 자동 | 모든 프로젝트 |
...
문서 요구나 상황별 절차는 해당될 때만 로드되는 cycle·스킬의 몫입니다. 매 세션 + 매 압축마다
