...
- **계획서 사이클**은 `rules/cycle.md` 에 있고, `/draft` 를 부르거나 `sprint` 스킬이 걸릴 때만
...
| **cycle** | 계획서 → 완주 → 적대 리뷰 → 보고, 문서 경로, `HANDOFF.md` | `/draft` 또는 스킬 호출 시 | 쓰는 리포에만 |
...
| 명령 | 하는 일 |
|---|---|
| `/draft <작업 설명>` | 목표·범위 합의 → `docs/plans/draft/task_NNN.md` 작성 → 승인 대기 |
